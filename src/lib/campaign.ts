...
export function resolveCampaignOpKind(kind: unknown): CampaignOpKindValue {
  ...
  if (value === "PMAX_CREATE") {
    throw Object.assign(
      new Error("PMAX_CREATE uses the Performance Max draft APIs, not the Phase 1 Search shell."),
      {
        status: 400,
        info: {
          kind: "validation",
          hint: "POST /api/ads/pmax/drafts then validate / apply. POST /api/ads/campaigns stays SEARCH_CREATE.",
        },
      },
    );
  }
  ...
  if (value === "VIDEO_CREATE") {
    throw Object.assign(
      new Error("VIDEO_CREATE uses the Video draft APIs, not the Phase 1 Search shell."),
      {
        status: 400,
        info: {
          kind: "validation",
          hint: "POST /api/ads/video/drafts then validate / apply. POST /api/ads/campaigns stays SEARCH_CREATE.",
        },
      },
    );
  }
  if (value === "SHOPPING_CREATE") {
    throw Object.assign(
      new Error("SHOPPING_CREATE uses the Shopping draft APIs, not the Phase 1 Search shell."),
      {
        status: 400,
        info: {
          kind: "validation",
          hint: "POST /api/ads/shopping/drafts then validate / apply. POST /api/ads/campaigns stays SEARCH_CREATE.",
        },
      },
    );
  }
  if (value !== "SEARCH_CREATE") {
    throw Object.assign(
      new Error(
        `${value} is schema-ready but not implemented. Google Search, Display, Performance Max, Demand Gen, Video, and Shopping create are the live paths.`,
      ),
      {
        status: 400,
        info: {
          kind: "validation",
          hint: "Use SEARCH_CREATE on /api/ads/campaigns, or the Display / Performance Max / Demand Gen / Video / Shopping wizard drafts. Other CampaignOpKind values are stubs.",
        },
      },
    );
  }
