...
export type AssistantContextCampaign = {
  ...
  source: "external_entity" | "search_draft";
};
...
export function mockAssistantTurn(input: {
  ...
  const url =
    ...
    input.pack.draft?.adGroups[0]?.ads[0]?.finalUrl ??
    null;
  const budgetMicros =
  ...
  const canFillTree = Boolean(brand || url || (input.message.trim().length > 12 && draftLooksEmpty(input.pack.draft)));
  if (canFillTree) {
    ...
    patch.adGroups = [
      {
        ...
        defaultBidMicros: input.pack.draft?.adGroups[0]?.defaultBidMicros ?? 1_000_000,
        sortOrder: 0,
  ...
  if (!url && !input.pack.draft?.adGroups[0]?.ads[0]?.finalUrl) {
    questions.push({
      id: "landing_url",
