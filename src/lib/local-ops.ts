...
import { mergeLocalDraftPatch } from "./assistant";
import {
  assertApplyConfirm,
...
export type LocalDraftRecord = LocalCampaignDraft & {
  externalAccount: { externalId: string };
  locations: LocalLocationDraft[];
  adGroups: Array<LocalAdGroupDraft & { ads: LocalAdDraft[] }>;
  targets: LocalTargetDraft[];
  campaignOps?: CampaignOp[];
};

export type LocalDraftView = LocalDraftClientView;
...
export function toLocalDraftView(row: LocalDraftRecord): LocalDraftView {
  return {
    id: row.id,
    ...
    locations: row.locations.map((item) => ({
      id: item.id,
      kind: item.kind,
...
export function localDraftViewToTree(view: LocalDraftView): LocalDraftTree {
  return toTree(view);
}

function toTree(view: LocalDraftView): LocalDraftTree {
  return {
    customerId: view.customerId,
    externalAccountId: view.externalAccountId,
    name: view.name,
    dailyBudgetMicros: Number(view.dailyBudgetMicros),
    biddingStrategy: view.biddingStrategy as LocalDraftTree["biddingStrategy"],
    goal: view.goal as LocalDraftTree["goal"],
    ...
    locations: view.locations.map((item) => ({
      kind: item.kind as LocalDraftTree["locations"][number]["kind"],
      valueText: item.valueText,
      placeIdText: item.placeIdText,
      addressText: item.addressText,
      included: item.included,
    ...
    targets: view.targets.map((target) => ({
      type: target.type as LocalDraftTree["targets"][number]["type"],
      valueText: target.valueText,
      criterionText: target.criterionText,
      included: target.included,
    })),
...
async function replaceDraftChildren(draftId: string, tree: LocalDraftTree) {
  const db = prisma();
  await db.localTargetDraft.deleteMany({ where: { draftId } });
  await db.localLocationDraft.deleteMany({ where: { draftId } });
  await db.localAdGroupDraft.deleteMany({ where: { draftId } });
  if (tree.locations.length) {
    await db.localLocationDraft.createMany({
      data: tree.locations.map((item, index) => ({
        draftId,
        kind: item.kind,
        valueText: item.valueText,
        placeIdText: item.placeIdText,
  ...
  for (const [index, group] of tree.adGroups.entries()) {
    await db.localAdGroupDraft.create({
      data: {
        draftId,
        name: group.name,
  ...
  if (tree.targets.length) {
    await db.localTargetDraft.createMany({
...
async function loadDraftOrThrow(id: string): Promise<LocalDraftRecord> {
  const ctx = await ensurePlatformContext();
  const row = await prisma().localCampaignDraft.findFirst({
    where: { id, organizationId: ctx.org.id },
    include: { ...draftInclude, externalAccount: true },
  });
  if (!row) {
    throw Object.assign(new Error("Local campaign draft not found."), {
      status: 404,
...
export async function listLocalDrafts(): Promise<LocalDraftView[]> {
  const ctx = await ensurePlatformContext();
  const rows = await prisma().localCampaignDraft.findMany({
    where: { organizationId: ctx.org.id },
    include: { ...draftInclude, externalAccount: true },
    orderBy: { updatedAt: "desc" },
    take: 50,
  });
  return rows.map((row) => toLocalDraftView(row));
}

export async function getLocalDraft(id: string): Promise<LocalDraftView> {
  return toLocalDraftView(await loadDraftOrThrow(id));
}

export async function createLocalDraft(body: unknown): Promise<LocalDraftView> {
  const tree = parseLocalDraftWrite(body);
  const ctx = await ensurePlatformContext();
  const external = await resolveExternalAccount({
    customerId: tree.customerId,
    externalAccountId: tree.externalAccountId,
  });
  const draft = await prisma().localCampaignDraft.create({
    data: {
  ...
  await writeAudit({
    ...
    action: "local_draft.created",
    resourceType: "LOCAL_CAMPAIGN_DRAFT",
    resourceId: draft.id,
...
export async function updateLocalDraft(id: string, body: unknown): Promise<LocalDraftView> {
  ...
  if (existing.statusDraft === "APPLIED") {
    throw Object.assign(new Error("Applied drafts are read-only."), {
      status: 400,
      info: { kind: "validation", hint: "Create a new draft to change an applied Local campaign." },
    });
  }
  const tree = parseLocalDraftWrite(body);
  const external = await resolveExternalAccount({
  ...
  await prisma().localCampaignDraft.update({
    where: { id },
    data: {
      externalAccountId: external.id,
      name: tree.name,
...
export async function patchLocalDraft(id: string, patch: unknown): Promise<LocalDraftView> {
  const existing = await getLocalDraft(id);
  const merged = mergeLocalDraftPatch(toTree(existing), patch);
  return updateLocalDraft(id, merged);
}

export async function deleteLocalDraft(id: string): Promise<{ deleted: true }> {
  const existing = await loadDraftOrThrow(id);
  if (existing.statusDraft === "APPLIED") {
  ...
  await prisma().localCampaignDraft.delete({ where: { id } });
  const ctx = await ensurePlatformContext();
  await writeAudit({
    organizationId: ctx.org.id,
    actorUserId: ctx.user.id,
    action: "local_draft.deleted",
    resourceType: "LOCAL_CAMPAIGN_DRAFT",
    resourceId: id,
  });
  return { deleted: true };
...
export async function validateOrApplyLocalDraft(
  id: string,
  body: unknown,
  mode: "validate" | "apply",
): Promise<{
  ...
  applied: boolean;
  status: "PAUSED";
  request: ReturnType<typeof buildLocalDraftMutate>;
  response: unknown;
  source: "live" | "mock";
  campaignOpId: string;
}> {
  ...
  const draft = await loadDraftOrThrow(id);
  const tree = parseLocalDraftTree({
    ...toTree(toLocalDraftView(draft)),
    ...raw,
    customerId: draft.externalAccount.externalId,
    status: raw.status ?? "PAUSED",
  });
  const request = buildLocalDraftMutate(tree, dryRun);
  const env = getEnv();
  const ctx = await ensurePlatformContext();
  const provider = await requireProvider(GOOGLE_ADS_SLUG);
    ...
    if (result.success && !request.validateOnly) {
      await prisma().changeRequest.create({
        data: {
          ...
          campaignOpId: campaignOp.id,
          requestedById: ctx.user.id,
          summary: `Apply PAUSED Local campaign ${tree.name}`,
          approved: true,
          reviewedAt: new Date(),
        },
      });
    ...
    await writeAudit({
      ...
      action: result.success
        ? request.validateOnly
          ? "local_draft.validated"
          : "local_draft.applied_paused"
        : "local_draft.failed",
      resourceType: "LOCAL_CAMPAIGN_DRAFT",
      resourceId: draft.id,
      metadata: {
  ...
  if (env.mockMode) {
    const response = {
      ...
      note: request.validateOnly
        ? "Dry-run: full Local tree validated locally. Nothing was sent to Google Ads."
        : "Mock apply: would create a PAUSED Local campaign tree. No live mutate executed.",
      mutateOperationResponses: request.validateOnly
        ? []
        : [{ campaignResult: { resourceName: `customers/${tree.customerId}/campaigns/mock-local-${draft.id}` } }],
    };
    const names = extractLocalResourceNames(response);
    await persistResult({
      response,
      source: "mock",
      success: true,
    ...
    return {
      draft: await getLocalDraft(draft.id),
      dryRun: request.validateOnly,
