"use client";

import { useCallback, useEffect, useState } from "react";

import type { ListingSyncCounts, SyncedCampaignView, SyncJobView } from "@/lib/types";

type ListingsResponse = {
  ok: boolean;
  campaigns?: SyncedCampaignView[];
  lastJob?: SyncJobView | null;
  source?: string;
  error?: string;
  hint?: string;
  warnings?: string[];
  dryRun?: boolean;
  persisted?: boolean;
  job?: SyncJobView;
  counts?: ListingSyncCounts;
};

function countCampaigns(campaigns: SyncedCampaignView[]): ListingSyncCounts {
  let adGroups = 0;
  let ads = 0;
  let keywords = 0;
  for (const campaign of campaigns) {
    adGroups += campaign.adGroups.length;
    for (const group of campaign.adGroups) {
      ads += group.ads.length;
      keywords += group.keywords.length;
    }
  }
  return { campaigns: campaigns.length, adGroups, ads, keywords };
}

export function ListingsPanel({
  customerId,
  connected,
  onFinished,
}: {
  customerId: string;
  connected: boolean;
  onFinished: () => Promise<void> | void;
}) {
  const [campaigns, setCampaigns] = useState<SyncedCampaignView[]>([]);
  const [lastJob, setLastJob] = useState<SyncJobView | null>(null);
  const [preview, setPreview] = useState(false);
  const [dryRun, setDryRun] = useState(true);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [note, setNote] = useState<string | null>(null);
  const [expanded, setExpanded] = useState<string | null>(null);

  const loadCache = useCallback(async () => {
    if (!connected || !customerId) {
      setCampaigns([]);
      setLastJob(null);
      setPreview(false);
      return;
    }
    const res = await fetch(`/api/ads/listings?customerId=${encodeURIComponent(customerId)}`, {
      cache: "no-store",
    });
    const json = (await res.json()) as ListingsResponse;
    if (!json.ok) {
      setError([json.error, json.hint].filter(Boolean).join(" — ") || "Unable to load listings cache.");
      return;
    }
    setError(null);
    setCampaigns(json.campaigns ?? []);
    setLastJob(json.lastJob ?? null);
    setPreview(false);
  }, [connected, customerId]);

  useEffect(() => {
    void loadCache();
  }, [loadCache]);

  async function runSync() {
    if (!customerId) return;
    setBusy(true);
    setError(null);
    try {
      const res = await fetch("/api/ads/listings/sync", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ customerId, dryRun }),
      });
      const json = (await res.json()) as ListingsResponse;
      if (!json.ok) {
        setError([json.error, json.hint].filter(Boolean).join(" — ") || "Sync failed.");
        return;
      }
      setCampaigns(json.campaigns ?? []);
      setLastJob(json.job ?? json.lastJob ?? null);
      setPreview(Boolean(json.dryRun));
      const counts = json.counts ?? countCampaigns(json.campaigns ?? []);
      setNote(
        json.dryRun
          ? `Dry-run preview: ${counts.campaigns} campaigns, ${counts.adGroups} ad groups, ${counts.ads} ads, ${counts.keywords} keywords. Neon cache was not written.`
          : `Cached ${counts.campaigns} campaigns (${counts.adGroups} ad groups). Read-only — live Ads status was not changed.`,
      );
      await onFinished();
    } finally {
      setBusy(false);
    }
  }

  const counts = countCampaigns(campaigns);

  return (
    <section className="rounded-2xl border border-ink-700 bg-ink-900 p-5" data-testid="ops-listings">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg text-white">Synced listings</h2>
          <p className="mt-1 text-sm text-moss-400">
            Read-only Google Ads pull into Neon{" "}
            <code className="font-mono text-moss-300">SyncedCampaign</code> / AdGroup / Ad / Keyword.
            Reuses <code className="font-mono text-moss-300">SyncJob</code>. Never enables or unpauses
            live campaigns.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <label className="flex items-center gap-2 font-mono text-xs text-moss-400">
            <input
              type="checkbox"
              className="accent-lime-400"
              checked={dryRun}
              onChange={(event) => setDryRun(event.target.checked)}
              data-testid="ops-listings-dry-run"
            />
            Dry-run (default)
          </label>
          <button
            type="button"
            data-testid="ops-listings-sync"
            onClick={() => void runSync()}
            disabled={!connected || !customerId || busy}
            className="rounded-lg bg-lime-400 px-4 py-2 text-sm font-medium text-ink-950 hover:bg-lime-500 disabled:opacity-50"
          >
            {busy ? "Syncing…" : dryRun ? "Preview sync" : "Sync to cache"}
          </button>
        </div>
      </div>

      {error ? <p className="mt-3 text-sm text-coral-400">{error}</p> : null}
      {note ? <p className="mt-3 text-sm text-amber-400">{note}</p> : null}
      {preview ? (
        <p className="mt-2 font-mono text-xs text-lime-400">Preview only — not written to Neon.</p>
      ) : null}
      {lastJob ? (
        <p className="mt-2 font-mono text-xs text-moss-500">
          Last SyncJob {lastJob.status.toLowerCase()} · {lastJob.dryRun ? "dry-run" : "cached"} ·
          read-only {lastJob.readOnly ? "yes" : "no"} · {lastJob.createdAt}
          {lastJob.counts
            ? ` · ${lastJob.counts.campaigns}c / ${lastJob.counts.adGroups ?? 0}g / ${lastJob.counts.ads ?? 0}a / ${lastJob.counts.keywords ?? 0}k`
            : ""}
        </p>
      ) : null}

      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat label="Campaigns" value={counts.campaigns} />
        <Stat label="Ad groups" value={counts.adGroups} />
        <Stat label="Ads" value={counts.ads} />
        <Stat label="Keywords" value={counts.keywords} />
      </div>

      <div className="mt-4 overflow-x-auto">
        <table className="w-full min-w-[720px] text-left text-sm" data-testid="ops-listings-table">
          <thead className="text-moss-500">
            <tr>
              <th className="pb-2 font-medium">Campaign</th>
              <th className="pb-2 font-medium">Type</th>
              <th className="pb-2 font-medium">Status</th>
              <th className="pb-2 font-medium">Children</th>
            </tr>
          </thead>
          <tbody>
            {!customerId ? (
              <tr>
                <td colSpan={4} className="py-6 text-moss-500">
                  Select an accessible customer above, then preview or cache listings.
                </td>
              </tr>
            ) : campaigns.length === 0 ? (
              <tr>
                <td colSpan={4} className="py-6 text-moss-500">
                  {connected
                    ? "No cached listings yet. Preview sync (dry-run) is the default."
                    : "Connect Google Ads to sync listings."}
                </td>
              </tr>
            ) : (
              campaigns.map((campaign) => {
                const open = expanded === campaign.externalId;
                const childCount = `${campaign.adGroups.length} groups`;
                return (
                  <CampaignRows
                    key={campaign.externalId}
                    campaign={campaign}
                    open={open}
                    childCount={childCount}
                    onToggle={() =>
                      setExpanded((current) =>
                        current === campaign.externalId ? null : campaign.externalId,
                      )
                    }
                  />
                );
              })
            )}
          </tbody>
        </table>
      </div>
    </section>
  );
}

function CampaignRows({
  campaign,
  open,
  childCount,
  onToggle,
}: {
  campaign: SyncedCampaignView;
  open: boolean;
  childCount: string;
  onToggle: () => void;
}) {
  return (
    <>
      <tr className="border-t border-ink-700">
        <td className="py-3">
          <button type="button" onClick={onToggle} className="text-left text-white hover:underline">
            {campaign.name}
          </button>
          <span className="mt-1 block font-mono text-xs text-moss-500">{campaign.externalId}</span>
        </td>
        <td className="py-3 font-mono text-xs text-moss-300">
          {campaign.advertisingChannelType ?? "—"}
        </td>
        <td className="py-3 font-mono text-xs">{campaign.status ?? "—"}</td>
        <td className="py-3 text-xs text-moss-400">{childCount}</td>
      </tr>
      {open
        ? campaign.adGroups.map((group) => (
            <tr key={group.externalId} className="border-t border-ink-800 bg-ink-950/60">
              <td className="py-2 pl-6 text-moss-300" colSpan={2}>
                <span className="text-white">{group.name}</span>
                <span className="mt-1 block font-mono text-xs text-moss-500">
                  {group.ads.length} ads · {group.keywords.length} keywords
                  {group.keywords[0] ? ` · ${group.keywords[0].text}` : ""}
                </span>
              </td>
              <td className="py-2 font-mono text-xs">{group.status ?? "—"}</td>
              <td className="py-2 text-xs text-moss-500">{group.type ?? "ad group"}</td>
            </tr>
          ))
        : null}
    </>
  );
}

function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-ink-700 bg-ink-950 px-3 py-2">
      <p className="text-xs text-moss-500">{label}</p>
      <p className="font-mono text-xl text-white">{value}</p>
    </div>
  );
}
