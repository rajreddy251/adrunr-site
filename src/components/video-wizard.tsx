...
import type { AdsAccountView, VideoDraftClientView } from "@/lib/types";
...
const STEPS = [
  { id: "V0", title: "Account" },
  { id: "V1", title: "Basics" },
  { id: "V2", title: "Creatives" },
  { id: "V3", title: "Audiences / geo" },
  { id: "V4", title: "Bidding" },
  { id: "V5", title: "Review" },
  { id: "V6", title: "Result" },
] as const;
...
function emptyAd(): WizardAd {
  return {
    ...
    assets: [
      { kind: "YOUTUBE_VIDEO", urlText: DEFAULT_YOUTUBE_VIDEO },
      { kind: "COMPANION_BANNER", urlText: DEFAULT_COMPANION_BANNER },
    ],
  };
}
...
function emptyGroup(index: number): WizardAdGroup {
  return {
    name: `Video ad group ${index + 1}`,
    defaultBidDollars: "1.00",
    ads: [emptyAd()],
  };
}

export type VideoWizardHandle = {
  getCustomerId: () => string;
  getDraftId: () => string | null;
  persist: () => Promise<string | null>;
  applyDraft: (draft: VideoDraftClientView) => void;
};

export const VideoWizard = forwardRef<
  VideoWizardHandle,
  {
...
>(function VideoWizard({ accounts, connected, onFinished }, ref) {
  ...
  const [name, setName] = useState("Adrunr paused Video");
  ...
  const [formats, setFormats] = useState<WizardFormats>({
    inStream: true,
    bumper: false,
    inFeed: true,
    shorts: true,
  ...
  function applyDraft(draft: VideoDraftClientView) {
    const next = hydrateVideoWizardFromDraft(draft);
    ...
    setFormats({
      inStream: next.inStream,
      bumper: next.bumper,
      inFeed: next.inFeed,
      shorts: next.shorts,
      outstream: next.outstream,
    });
  ...
  useImperativeHandle(
    ...
    [customerId, draftId, name, budgetDollars, startDate, endDate, groups, targets, audiences, formats],
  );

  async function persistDraft(): Promise<string | null> {
    const body = payload();
    const res = await fetch(draftId ? `/api/ads/video/drafts/${draftId}` : "/api/ads/video/drafts", {
  ...
  async function runAction(kind: "validate" | "apply") {
    ...
    const res = await fetch(`/api/ads/video/drafts/${id}/${kind}`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
  ...
  return (
    <section className="rounded-2xl border border-ink-700 bg-ink-900 p-5" data-testid="video-wizard">
      <h2 className="text-lg text-white">Video wizard (PAUSED)</h2>
      <p className="mt-1 text-sm text-moss-400">
        Schema v1.8 drafts in Neon. Ad groups + YouTube video responsive ads + USER_LIST audiences.
      ...
      <ol className="mt-4 flex flex-wrap gap-2" aria-label="Video wizard steps">
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        {step === 5 ? (
          <>
            <button
              type="button"
              data-testid="video-wizard-validate"
            ...
            <button
              type="button"
              data-testid="video-wizard-apply"
...
function BasicsStep({
  ...
  startDate: string;
  endDate: string;
  formats: WizardFormats;
  onName: (value: string) => void;
  onBudget: (value: string) => void;
  onStart: (value: string) => void;
  onEnd: (value: string) => void;
  onFormats: (value: WizardFormats) => void;
}) {
  const formatRows: Array<{ key: keyof WizardFormats; label: string }> = [
    { key: "inStream", label: "YouTube In-stream" },
    { key: "bumper", label: "Bumper" },
    { key: "inFeed", label: "YouTube In-feed" },
    { key: "shorts", label: "YouTube Shorts" },
    { key: "outstream", label: "Outstream" },
  ];
  return (
    <div className="space-y-5">
      ...
      <fieldset>
        <legend className="text-sm text-white">V1 · Video formats</legend>
        <p className="mt-1 text-xs text-moss-500">Stored on the draft and sent as videoAdInventoryControl on apply.</p>
          ...
          {formatRows.map((row) => (
            <label key={row.key} className="flex items-center gap-2 text-sm text-moss-300">
              <input
                ...
                checked={formats[row.key]}
                onChange={(event) => onFormats({ ...formats, [row.key]: event.target.checked })}
              />
              {row.label}
            </label>
          ))}
  ...
  return (
    <div className="space-y-5">
      <p className="text-sm text-moss-400">
        V2 · Ad group + video responsive ad + YouTube asset. {VIDEO_HEADLINE_MAX} headlines
        max, {VIDEO_DESCRIPTION_MAX} descriptions max.
      </p>
      {groups.map((group, groupIndex) => (
        <div key={`vid-${groupIndex}`} className="space-y-4 rounded-xl border border-ink-700 bg-ink-950 p-4">
          ...
          {group.ads.map((ad, adIndex) => (
            <div key={`vad-${groupIndex}-${adIndex}`} className="space-y-3">
              ...
              <Field label="Long headline (optional)">
                <input
              ...
              <div className="space-y-2">
                <p className="text-xs text-moss-500">Assets (URLs stored as TEXT) — YouTube video required</p>
                {ad.assets.map((asset, assetIndex) => (
                  <div key={`asset-${assetIndex}`} className="grid gap-2 md:grid-cols-[12rem_1fr]">
                    ...
                    >
                      ...
                      <option value="COMPANION_BANNER">Companion banner</option>
                      <option value="MARKETING_IMAGE">Image</option>
                    </select>
                    <input
                      value={asset.urlText}
                      onChange={(event) => {
...
function AudienceGeoStep({
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <fieldset>
        <legend className="text-sm text-white">V3 · Geo (MVP)</legend>
        <div className="mt-3 space-y-2">
      ...
      <fieldset>
        <legend className="text-sm text-white">Video audiences</legend>
        <p className="mt-1 text-xs text-moss-500">
          These are Video USER_LIST audiences, not a separate campaign type.
        </p>
        <div className="mt-3 space-y-2">
...
function ReviewStep({
  ...
  const formatLabels = [
    formats.inStream ? "YouTube In-stream" : null,
    formats.bumper ? "Bumper" : null,
    formats.inFeed ? "YouTube In-feed" : null,
    formats.shorts ? "YouTube Shorts" : null,
    formats.outstream ? "Outstream" : null,
  ].filter(Boolean);
  return (
    <div className="space-y-4 text-sm">
      <p className="text-moss-400">
        V5 · Review, then validate (preferred) or type {CONFIRM_PAUSED_PHRASE} to apply PAUSED.
      </p>
      <dl className="grid gap-3 md:grid-cols-2">
        ...
        <div>
          ...
          <dd className="text-white">
            {name} · ${budgetDollars}/day · PAUSED · Video · Manual CPV
          </dd>
        </div>
        <div>
        ...
        <div>
          <dt className="text-moss-500">Formats</dt>
          <dd className="text-moss-300">{formatLabels.join(", ") || "none"}</dd>
        </div>
        <div>
      ...
      <ul className="space-y-2 text-moss-300">
        {groups.map((group) => (
          <li key={group.name} className="rounded-lg border border-ink-700 bg-ink-950 px-3 py-2">
            <span className="text-white">{group.name}</span> · {group.ads.length} Video ad ·{" "}
      ...
      <label className="block">
        ...
        <input
          ref={confirmRef}
          data-testid="video-wizard-confirm"
          value={confirmPhrase}
...
function ResultStep({ result, draftId }: { result: WizardResult | null; draftId: string | null }) {
  if (!result) {
    return <p className="text-sm text-moss-500">V6 · Run validate or Create PAUSED from review to see a result.</p>;
  }
  return (
    <div className="space-y-3">
      <p className="text-sm text-white">
        V6 · {result.ok ? (result.applied ? "Applied PAUSED" : "Validated") : "Failed"}{" "}
