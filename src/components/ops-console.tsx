  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-7xl flex-col gap-6 px-5 py-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-moss-400">
            Provider-agnostic foundation (Schema v1.15). Google Ads Search + Display + Performance Max
            + Demand Gen + Video + Shopping + App + Hotel + Local + Local Services wizards + fill-first assistant + read-only listings and budget/spend sync + safe campaign edit + import-to-draft; other providers are seeded stubs. MCC{" "}
      ...
      <footer className="pb-8 text-xs text-moss-500">
        Adrunr · Schema v1.15 · Neon Postgres + Prisma · encrypted OAuth columns · no file tokens · no
        JSONB · listings + metrics sync are read-only · safe edit never enables · import stays PAUSED-path · no spend/enable path
      </footer>
    </div>
  );
}
