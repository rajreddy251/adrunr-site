...
- **Safe campaign edit never enables spend.** `POST /api/ads/edits/drafts/:id/validate` is validateOnly first. Apply requires typing `EDIT SAFE` and only mutates name, budget, bids, or GEO/LANGUAGE targeting. Status `ENABLED` / enable / unpause / go-live are refused. Dry-run is the default on `/ops/campaigns/:id/edit`. There is no Enable CTA on Overview or Edit.
- **Import is draft-only.** `POST /api/ads/imports` maps a cached `SyncedCampaign` into a create-type draft (`SearchCampaignDraft`, Display, PMax, …). Dry-run preview is the default. Import never enables, unpauses, or calls `googleAds:mutate`. Imported drafts stay on the PAUSED create path (`CREATE PAUSED`).
- **Performance reports are read-only.** `GET /api/ads/reports` lists persisted `CampaignReportJob` rows. `POST /api/ads/reports` (or `/sync`) builds a spend / clicks / impressions / conversions rollup from `CampaignMetricSnapshot` cache and/or GAQL `googleAds:search`. Dry-run (preview, no persist) is the default. Persist only when `dryRun: false`. Reports never enable, unpause, mutate live Ads, or spend. Cached `ENABLED` is a snapshot only.
- **Campaign chat cannot spend.** The Search assistant on campaign focus routes and the create wizard is fill-first only. It cannot Validate, Apply, Create PAUSED, Enable, Pause, or Delete. Proposed fills stay in the form until the operator uses those CTAs.
- **No spend without an explicit confirm.** Applying a paused create requires typing `CREATE PAUSED` (enforced server-side). Applying a safe edit requires typing `EDIT SAFE`. Pause / Delete confirms require `PAUSE CAMPAIGN` / `DELETE CAMPAIGN`; live mutate stays gated (`Coming soon — use Google Ads`) until a safe path ships.
...
5. **Search Campaign Overview + Safe edit (P2 / Slice B)** — `/ops/campaigns/:id` is the workspace home for one cached campaign (name, type Search, status snapshot). Action bar: Edit / Pause / Delete / Open chat. Never Enable. `/ops/campaigns/:id/edit` hosts the CampaignEditDraft form (dry-run default). Validate is lime-primary while dry-run is on. Apply stays locked until the form is valid, dry-run is off, and the phrase is exactly `EDIT SAFE`. `GET|POST /api/ads/edits/drafts` stores a `CampaignEditDraft` (TEXT children for field kinds, bids, GEO/LANGUAGE). Validate is `validateOnly`. Apply mutates name / budget / bids / targeting-safe fields only. Status `ENABLED`, enable, unpause, and go-live are refused. `ADRUNR_MOCK=1` validates or pretends to apply locally — no live spend.
5b. **Campaign chat (Slice D)** — Open chat on Overview `/ops/campaigns/:id`, Safe edit `/ops/campaigns/:id/edit`, and Search create `/ops/campaigns/new/search` (S1–S7). ~380px right split (chip **Chat with this campaign** when collapsed). Threads scope to open campaign `:id` or `draftId` + `customerId` + `kind: SEARCH` via existing `/api/assistant/turn`. Switching campaigns rebinds the thread. Fill-first / gap-only: chat proposes form fills with **Proposed by chat** markers and toasts that it did **not** apply. User still uses form **Validate (dry-run)** / **EDIT SAFE** / **CREATE PAUSED**. Chat hard-refuses Validate, Apply, Create PAUSED, Enable, Pause, and Delete (coral `Refused: {action}. Use the form CTAs.`). Persistent subtitle: chat cannot Validate, Apply, Create PAUSED, or Enable. Send is lime primary. No Enable CTA. Not a global assistant on `/ops` list or shell utilities.
5a. **Pause / Delete confirms (Slice C)** — `/ops/campaigns/:id/pause` requires typing `PAUSE CAMPAIGN`. Amber armed primary. Copy: Adrunr will not enable later; re-enable only in Google Ads. `/ops/campaigns/:id/delete` requires typing `DELETE CAMPAIGN`. Coral destructive. No safe Google Ads pause/delete mutate is shipped yet — after the phrase the primary stays gated with `Coming soon — use Google Ads` (dry-run preview only). Never Enable. Never call status `ENABLED`.
6. **Import to draft (P3)** — `/ops/import` Import to draft. Pick a cached `SyncedCampaign` → preview mapping → import. `GET|POST /api/ads/imports` writes a `CampaignImportJob` (TEXT request/response/preview). `dryRun` defaults to true (preview, no draft write). Persist creates the matching create-type draft (`SearchCampaignDraft` for SEARCH, Display for DISPLAY, …) with provenance TEXT and never enables / unpauses live Ads. Open the matching wizard to validate / `CREATE PAUSED`. Stale `syncedCampaignId` is ignored when `campaignExternalId` matches the cache.
7. **Performance reports (P4)** — `/ops/reports` Performance reports after listings + metrics + edit + import. Date range + selected customer → Preview → Generate report. `GET /api/ads/reports` lists persisted `CampaignReportJob` rows. `POST /api/ads/reports` or `POST /api/ads/reports/sync` with `{ customerId, dryRun, dateFrom?, dateTo? }` (default `dryRun: true`, last 7 days) builds spend / clicks / impressions / conversions rollups from `CampaignMetricSnapshot` cache and/or GAQL `googleAds:search` (or `ADRUNR_MOCK` fixtures). Persist only when `dryRun: false`. Never calls `googleAds:mutate`. Never enables, unpauses, or spends. Cached `ENABLED` is labeled snapshot-only.
8. **Search wizard (Phase 2 / Slice E)** — `/ops/campaigns/new/search` S0 account → S1 basics → S2 ad groups → S3 keywords → S4 RSA → S5 geo/language → S6 Manual CPC → S7 review → Validate / Create PAUSED → S8 result. Entry: Campaigns **Create campaign → Search**, Import **Open Search draft**, optional hub card. Wizard body is not stacked on hub/list. Dry-run / `validateOnly` defaults ON (amber). **Validate (dry-run)** is lime primary. **Create PAUSED** stays locked until the phrase is exactly `CREATE PAUSED`, dry-run is off, and the draft is valid. Creates always PAUSED. No Enable. Drafts persist as `SearchCampaignDraft` (+ child rows). Campaign chat (Slice D) on S1–S7 scopes to `draftId` + `kind: SEARCH` and cannot Validate / Apply / Create PAUSED / Enable.
9. **Display wizard** — `/ops/campaigns` Display tab D0 account → D1 basics → D2 creatives/assets → D3 audiences/geo → D4 bidding → D5 review → Validate / Create PAUSED → D6 result. Drafts persist as `DisplayCampaignDraft` (+ ad groups, ads/assets, audiences, geo). Remarketing is a Display `USER_LIST` audience, not a campaign type. Same PAUSED + `CREATE PAUSED` rules. No enable path.
10. **Performance Max wizard** — `/ops/campaigns` Performance Max tab P0 account → P1 basics → P2 asset group → P3 signals/geo/listings → P4 bidding → P5 review → Validate / Create PAUSED → P6 result. Drafts persist as `PerformanceMaxCampaignDraft` (+ asset groups, assets, signals, optional listings, geo). Apply uses Maximize conversions. ALL_PRODUCTS listings apply only with a Merchant Center id; Sync listings is out of scope. Same PAUSED + `CREATE PAUSED` rules. No enable path.
11. **Demand Gen wizard** — `/ops/campaigns` Demand Gen tab G0 account → G1 basics/channels → G2 creatives/assets → G3 audiences/geo → G4 bidding → G5 review → Validate / Create PAUSED → G6 result. Drafts persist as `DemandGenCampaignDraft` (+ ad groups, multi-asset ads/assets, audiences, geo, channel checkboxes). Apply uses Maximize conversions + USER_LIST audiences. Same PAUSED + `CREATE PAUSED` rules. No enable path.
12. **Video wizard** — `/ops/campaigns` Video tab V0 account → V1 basics/formats → V2 creatives/YouTube assets → V3 audiences/geo → V4 bidding → V5 review → Validate / Create PAUSED → V6 result. Drafts persist as `VideoCampaignDraft` (+ ad groups, video responsive ads/YouTube assets, audiences, geo, format inventory). Apply uses Manual CPV + USER_LIST audiences. Same PAUSED + `CREATE PAUSED` rules. No enable path.
13. **Shopping wizard** — `/ops/campaigns` Shopping tab H0 account → H1 basics/Merchant Center → H2 product groups/listings → H3 geo → H4 bidding → H5 review → Validate / Create PAUSED → H6 result. Drafts persist as `ShoppingCampaignDraft` (+ ad groups, ALL_PRODUCTS product groups, optional listings, geo, Merchant Center / sales country / priority). Apply uses Manual CPC + Merchant Center + ALL_PRODUCTS. UNIT / SUBDIVISION and tROAS stay stored. Same PAUSED + `CREATE PAUSED` rules. No enable path.
14. **App wizard** — `/ops/campaigns` App tab A0 account → A1 basics/goal/CPI → A2 app/platforms → A3 creatives → A4 geo → A5 review → Validate / Create PAUSED → A6 result. Drafts persist as `AppCampaignDraft` (+ platforms/app ids, ad groups, app ads/assets, geo, INSTALLS goal). Apply uses TARGET_CPA + INSTALLS + the first included Android (or iOS) app id. In-app actions / Maximize conversions / tROAS stay stored. Same PAUSED + `CREATE PAUSED` rules. No enable path.
15. **Hotel wizard** — `/ops/campaigns` Hotel tab T0 account → T1 basics/Hotel Center → T2 ad group + ALL_HOTELS → T3 geo → T4 bidding → T5 review → Validate / Create PAUSED → T6 result. Drafts persist as `HotelCampaignDraft` (+ ad groups, ALL_HOTELS listings, geo, Hotel Center / percent-CPC ceiling). Apply uses PERCENT_CPC + Hotel Center + ALL_HOTELS. UNIT / COMMISSION / MANUAL_CPC / tROAS stay stored. Same PAUSED + `CREATE PAUSED` rules. No enable path.
16. **Local wizard** — `/ops/campaigns` Local tab L0 account → L1 basics/goal → L2 location → L3 ad → L4 geo → L5 review → Validate / Create PAUSED → L6 result. Drafts persist as `LocalCampaignDraft` (+ locations, ad groups, local ads, geo, STORE_VISITS). Apply uses MAXIMIZE_CONVERSIONS + STORE_VISITS + a location + one ad. STORE_SALES stays stored. Same PAUSED + `CREATE PAUSED` rules. No enable path.
...
- `GET|POST /api/ads/imports` — preview (default) or persist a cached SyncedCampaign as a create-type draft
- `GET /api/ads/imports/:id` — CampaignImportJob + preview TEXT
- `GET /api/ads/reports` — persisted CampaignReportJob + rows + spend/clicks/impressions/conversions rollup
- `POST /api/ads/reports` — read-only report from metrics cache and/or googleAds:search; `dryRun` default true (preview, no persist)
- `POST /api/ads/reports/sync` — same as POST /api/ads/reports
- `GET /api/ads/reports/:id` — CampaignReportJob + TEXT preview
