...
This repo is the **single production codebase**. Behavior is ported from the validated MVP (`rajreddy251/adrunr` @ `493d52c`) and evolved onto **Neon Postgres + Prisma Schema v1.6** (provider-agnostic). There is no `.data/tokens.json` path.
...
- New Search, Display, and Performance Max campaigns are created **PAUSED**. Dry-run (`validateOnly`) is the default and preferred path.
...
`CampaignOpKind` values: `SEARCH_CREATE`, `PMAX_CREATE`, `DISPLAY_CREATE`, `META_CAMPAIGN_CREATE`, `TIKTOK_CAMPAIGN_CREATE`, `LINKEDIN_CAMPAIGN_CREATE`, `GENERIC_MUTATE`. Google implements `SEARCH_CREATE`, `DISPLAY_CREATE`, and `PMAX_CREATE`. Other kinds are schema-ready stubs. Phase 1 `POST /api/ads/campaigns` stays Search-only.
...
`ADRUNR_MOCK=1` demos **Connect → list accounts → Search (S0–S8), Display (D0–D6), or Performance Max (P0–P6) wizard + fill-first assistant → validateOnly / Create PAUSED → audit trail** without live Google Ads or an LLM key. Tokens, drafts, threads, and accounts still persist in Neon.
...
6. **Wizard assistant** — panel beside Search, Display, or Performance Max. Paste a URL or brief; the turn builds a client-scoped context pack (org, client, ExternalAccounts, ExternalEntity/Search/Display/PMax drafts, current draft, thread, ClientMemory) and either patches draft fields and/or asks gap questions. Fill first — not a questionnaire. Chat **cannot** Validate, Apply, or enable and never calls those endpoints. `ADRUNR_MOCK=1` uses a heuristic fill when no LLM key is set.
7. **Paused / dry-run shell (Phase 1)** — `POST /api/ads/campaigns` with `{ customerId, name, dailyBudgetMicros, dryRun, confirmPhrase }` still works. Always `status: PAUSED`. `dryRun` (default `true`) sets `validateOnly`. `dryRun: false` still creates PAUSED only and **requires** `confirmPhrase` exactly `CREATE PAUSED`. Persists `CampaignOp` + `DryRunJob` (and `ChangeRequest` on apply). Search-only.
8. **GA4 stub** — `GET /api/ga4/report` runs a 7-day sessions + conversions sample and upserts a `google_analytics` ExternalAccount. Soft-fails if property id, scope, or API is missing.
9. **Other providers** — seeded only. Connect buttons are disabled stubs.
