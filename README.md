...
This repo is the **single production codebase**. Behavior is ported from the validated MVP (`rajreddy251/adrunr` @ `493d52c`) and evolved onto **Neon Postgres + Prisma Schema v1.15** (provider-agnostic). There is no `.data/tokens.json` path.
...
`ADRUNR_MOCK=1` demos **Connect → list accounts → read-only listings sync (dry-run preview or cache) → read-only budget/spend snapshots (dry-run preview or cache) → safe campaign edit (validateOnly / EDIT SAFE) → import cached campaign to a create-type draft (dry-run preview or persist) → Search (S0–S8), Display (D0–D6), Performance Max (P0–P6), Demand Gen (G0–G6), Video (V0–V6), Shopping (H0–H6), App (A0–A6), Hotel (T0–T6), Local (L0–L6), or Local Services (S0–S6) wizard + fill-first assistant → validateOnly / Create PAUSED → audit trail** without live Google Ads or an LLM key. Tokens, drafts, threads, synced listings, metric snapshots, edit drafts, import jobs, and accounts still persist in Neon. Import never enables spend.
